...
import { FormsModule, FormControl, FormGroup, Validators, FormBuilder, FormControlName } from '@angular/forms';
...
export class RegisterComponent implements OnInit {
  ...
  registerFormFields() {
    ...
    // const certainPassword = new FormControl('', [CustomValidators.equalTo(userPassword), ]);
    this.registerForm = new FormGroup({
      fullName: new FormControl('', [Validators.required, Validators.maxLength(30),
                                      Validators.minLength(3)]),
      userEmail: new FormControl('', [Validators.required, CustomValidators.email]),
      userPassword,
      confirmPassword: new FormControl('', [CustomValidators.equalTo(userPassword), ])
  ...
  // get passwordMatch() { return this.userPassword.value !== this.confirmPassword.value; }
  ...
  register() {
    if (this.registerForm.valid) {
      ...
      (data: any) => {
        ...
        this.getSweetAlert('Success', 'success' , 'A verification email has been sent', 'register');
      }, err => {
        if (err.code === 400) {
  ...
  getSweetAlert(title, type, text, route ) {
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.value) {
        if (route === 'register') {
          this.httpservice.resend(this.userEmail.value).subscribe(
            (data: any) => {
