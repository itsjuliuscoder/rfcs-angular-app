...
export class ResendComponent implements OnInit {
  ...
  ForgetPasswordFormField() {
    this.resendEmailForm = new FormGroup({
        forgetEmail: new FormControl('', [Validators.required, CustomValidators.email]),
    });
  }

  get forgetEmail() { return this.resendEmailForm.get('forgetEmail'); }
  ...
  resendEmail() {
    if (this.resendEmailForm.valid) {
      ...
      this.httpservice.resend(email).subscribe(
        (data: any) => {
  ...
  get getDisableForgetState() { return this.resendEmailForm.invalid || this.disableBtn; }
